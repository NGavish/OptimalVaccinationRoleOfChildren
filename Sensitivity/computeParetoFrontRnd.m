function computeParetoFrontRnd(susceptibilityFactor,maxPrct,VcPrct,betaVac,nuVac,effVac,vaccineRange,R0,recoveredprct,rndIdx)
...
fname=['./data/rnd',num2str(rndIdx),'_dataParetoFront_susFactor=',num2str(100*susceptibilityFactor),'_maxPrct',num2str(maxPrct),'_VcPrct=',num2str(10*VcPrct),'_betaVac=',num2str(betaVac*100),'_nuVac=',num2str(10*nuVac),'_recoveredPrct=',num2str(10*recoveredprct)];
...
weight=1+0.2*(rand(1,9)-0.5);
Cij=diag(weight)*Cij*diag(weight);
M2aux=diag(Ni)*Cij*diag(1./Ni);[V,d]=eig(M2aux);Cij =Cij/max(d(:));
