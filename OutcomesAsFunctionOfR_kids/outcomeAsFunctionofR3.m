...
if collectData
    sampleSize=5000;
    Rvec=3;unique([linspace(1,4,200) linspace(1.74,1.86,100) linspace(2.5,2.7,200) linspace(2.7,3.2,200) linspace(2.8,2.87,50) linspace(3,3.05,50) linspace(3,3.4,100)]);
    count=0
    M=numel(Rvec);
...
%               [result,overallInfected_nv_uniform(ix),overallFatality_uniform(ix),data_uniform{ix}]=computeFinalSize(uniformAllocation,adultAges,IFR,0,R0,Cij,Ni,r,v,infected0_v,infected0_nv,betaVac,effVac,1,1);
           ...
           save(['../data/R=3,OutcomesAsFuncOfR_',fname,'VcPrct_',num2str(10*VcPrct),'_',vaccineRange,'_',susFactorText]);
            toc
        end
    end
end
...
t=tiledlayout(3,2, 'TileSpacing','compact','Padding','none')
    ...
    for ix=1:numel(vaccineRangeVec)
        ...
        data=load(['../data/R=3,OutcomesAsFuncOfR_',fname,'VcPrct_',num2str(10*VcPrct),'_',vaccineRange,'_',susFactorText]);
