...
betaVac=0.2; effVac=0.75;betaVacB=0.15;betaVacC=0.25;
...
lg=legend(axHndl(2),[hndl(1,3),hndl(2,3) hline],'R_{\rm critical}^{20+}','R_{\rm critical}^{10+}','80% vaccine efficacy','fontsize',10,'Orientation','horizontal');
...
title(axHndl(2),'85% vaccine efficacy');
title(axHndl(3),'75% vaccine efficacy');
...
%lg = legend(nexttile(3),'Allocations along Pareto front','Allocations minimizing moratility','Allocations minimizing infections','Uniform allocation','Baseline curve','fontsize',10,'Orientation','horizontal');
    ...
    figure;
    scatter(assortivityOfChildren,Rherd_w0,12,'MarkerFaceColor',blue,'MarkerEdgeColor',blue);hold on;
    %scatter(prctOfChildren,Rherd_w1,'r.','markersize',12);
    
    text(assortivityOfChildren,Rherd_w0,countryList,'HorizontalAlignment','center','VerticalAlignment','bottom','color','k','fontsize',12);
    xlabel('Assortivity of children');ylabel('Herd immunity R');box on;
    return
