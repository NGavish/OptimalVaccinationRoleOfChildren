...
defineColors;
alreadyRecovered=recoveredprct*sum(data.Ni)/100;
...
if presentationOptions.presentScatterplot
    scatter1=scatter(data.sample_overallInfected/1e6,data.sample_overallMortality/1e6,8,'filled',markerType,'MarkerFaceColor',randColor,'MarkerEdgeColor',randColor)
    alpha(scatter1,alphaValue)
     hold on;
end

if presentationOptions.presentUniform
        hold on;plot(data.overallInfected_uniform/1e6,data.overallMortality_uniform/1e6,'d','color',blue,'markersize',5,'linewidth',2)
    text(data.overallInfected_uniform/1e6,data.overallMortality_uniform/1e6,{'Homogenous',['(',vaccineRangeTxt,')']},'HorizontalAlignment','center','VerticalAlignment','bottom','color',blue,'fontsize',12,'FontWeight','bold')
...
overallMortality=data.overallMortality/1e6;
