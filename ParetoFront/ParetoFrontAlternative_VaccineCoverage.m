...
fname=['dataParetoFront',num2str(10*VcPrct),'_leaky'];R0=3;betaVac=0.2;nuVac=1; effVac=0.75;
...
dataLeaky{1}=load('dataParetoFront_susFactor=100_maxPrct100_VcPrct=732_betaVac=2_nuVac=1_All_R=3');
dataLeaky{2}=load('dataParetoFront732_leaky_All_R=3');
dataLeaky{3}=load('dataParetoFront800_leaky_All_R=3');
dataLeaky{4}=load('dataParetoFront850_leaky_All_R=3');
%% Present Data
close all;
...
xlabel('w');title(['Allocation of vaccines among age groups - ',num2str(dataLeaky{jx}.VcPrct*sum(Ni(3:9))/sum(Ni),2),'% vaccine coverage']);
...
text(axVec{1},20,0.55,{'64% vaccine','coverage'},'fontweight','bold');
text(axVec{1},50,1,{'60% vaccine','coverage'},'fontweight','bold');
text(axVec{1},85,1.6,{'55% vaccine','coverage'},'fontweight','bold');
text(axVec{1},145,2.2,{'45% vaccine','coverage'},'fontweight','bold');
...
printGraph([fname,'_ParetoFrontVaccineCoverage'])
return
