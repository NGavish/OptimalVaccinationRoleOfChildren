function ParetoFrontAlternative_susceptibility(susceptibilityFactor)
addpath('/Users/nirgavish/Dropbox (Technion Dropbox)/My Shared Functions')
addpath('../Codes/AllCountries/AuxilaryFunctions/');
close all;
vaccineRange='All';
%% Parameters
collectData=false;maxPrct=100;%susceptibilityFactor=1;
...
VcPrct=73.2;  % Maximal vaccination per age group
...
R0=3;betaVac=0.2;nuVac=1; effVac=1-0.05/betaVac;fname=['dataParetoFront_susFactor',num2str(100*susceptibilityFactor)];
...
% R0=3;betaVac=0.5;nuVac=1; effVac=1-0.05/betaVac;fname=['dataParetoFront_betaVac05',num2str(10*VcPrct),'_leaky'];
...
dataLeaky{1}=load('dataParetoFront_susFactor100_All_R=3');dataLeaky{1}.overInfected=dataLeaky{1}.infectionVec;
dataLeaky{2}=load('dataParetoFront_susFactor150_All_R=3');
dataLeaky{3}=load('dataParetoFront_susFactor175_All_R=3');
dataLeaky{4}=load('dataParetoFront_susFactor200_All_R=3');
%% Present Data
close all;
...
overallInfected=dataLeaky{ix}.infectionVec/1e6;
...
%legend(hndl,'95% maximum vaccination in age group','85% maximum vaccination in age group','75% maximum vaccination in age group','65% maximum vaccination in age group','location','best'); 
legend(hndl,'Susceptiblity factor 1','Susceptiblity factor 1.5','Susceptiblity factor 1.75','Susceptiblity factor 2','location','best'); 
...
xlabel('w');title(['Allocation of vaccines among age groups - ',num2str(dataLeaky{jx}.susceptibilityFactor),' susceptibility factor']);
...
% text(axVec{1},20,0.55,{'90% VE','coverage'},'fontweight','bold');
% text(axVec{1},50,1,{'80% VE','coverage'},'fontweight','bold');
% text(axVec{1},85,1.6,{'70% VE','coverage'},'fontweight','bold');
% text(axVec{1},145,2.2,{'60% VE','coverage'},'fontweight','bold');
...
printGraph([fname,'_ParetoFrontSusceptiblityFactor']);
%printGraph([fname,'_ParetoFrontVaccineHesistancy'])
return
