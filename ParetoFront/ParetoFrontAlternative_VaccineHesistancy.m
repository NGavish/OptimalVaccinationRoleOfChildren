...
R0=3;betaVac=0.2;nuVac=1; effVac=1-0.05/betaVac;fname=['dataParetoFront_maxPrct',num2str(maxPrct),'_leaky'];
...
problem.options = optimoptions('fmincon','MaxFunctionEvaluations',5e4,'Display','none');%,'Algorithm','sqp');%,'Display','iter');
...
dataLeaky{1}=load('dataParetoFront_maxPrct95_leaky_All_R=3');
dataLeaky{2}=load('dataParetoFront_maxPrct85_leaky_All_R=3');
dataLeaky{3}=load('dataParetoFront_maxPrct75_leaky_All_R=3');
dataLeaky{4}=load('dataParetoFront_maxPrct65_leaky_All_R=3');
%% Present Data
close all;
...
xlabel('w');title(['Allocation of vaccines among age groups - ',num2str(dataLeaky{jx}.maxPrct),'% vaccination per age group']);
